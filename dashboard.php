...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search'])) {
        ...
        if (!empty($searchTerm)) {
            ...
            $query = "SELECT law_act, heading, content 
                      FROM laws 
                      WHERE law_act LIKE ? 
                         OR heading LIKE ? 
                         OR content LIKE ?";
...
<div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="content open">
            ...
            <nav class="navbar navbar-expand bg-secondary navbar-dark sticky-top px-4 py-0">
                ...
                <div class=" " style="margin-left: 45em !important;">
                <form class="  " method="POST">
                    <input class="form-control bg-dark border-0" type="text" placeholder="Search Laws"name="search" required>
                </form>
                </div>
        ...
        <?php if (!empty($results)): ?>
            <?php foreach ($results as $row): ?>
                <h3><?php echo htmlspecialchars($row['law_act']); ?></h3>
